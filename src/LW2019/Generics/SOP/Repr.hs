...
instance Generic BookInfo where
  ...
  to (SOP (Z (I name :* I isbn :* I price :* Nil))) = BookInfo name isbn price
  from (BookInfo name isbn price) = SOP (Z (I name :* I isbn :* I price :* Nil))

instance Generic QualName where
  type Code QualName = '[ '[ String], '[ String, QualName]]
